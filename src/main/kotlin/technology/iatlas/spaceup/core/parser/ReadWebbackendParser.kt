        ...
        splittedLines.forEach {
            ...
            when (splittedResponse.size) {
                ...
                2 -> { // NOT OK service
                    val web = splittedResponse[0]
                    val process = "NOT OK"
                    val service = splittedResponse[1]
                    configList.add(WebbackendConfiguration(web = web, process = process, service = service))
                }
                4 -> {
                    val web = splittedResponse[0]
                    ...
                    // sanitize credentials
                    val regex = Regex("://(.*:.*)@")
                    val service = splittedResponse[3].replace(regex, "://[hidden]:[hidden]@")
